localStorage.setItem("mb_last_s1", "Movie Quiz");
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // ===== Init state =====
  if (done && saved) {
    if (Array.isArray(saved.answers)) answers = saved.answers;
    clearProgressMovie();
    showResult(saved);
  } else {
  ...
  nextBtn.addEventListener("click", () => {
    ...
    const result = { total, correct, acc, answers: Array.isArray(answers) ? answers.slice() : [], name: p?.name || "Player", id, ts: Date.now() };
...
function injectBreadcrumb(){
  ...
  crumbs.innerHTML = `<a href="../../../index.html">All Seasons</a> / <a href="../index.html">Season 1</a> / <span>Movie Quiz</span>`;
...
function injectResultNavButtons(){
  ...
  if (!document.getElementById("backSeasonBtn")){
    ...
    a.href = "../index.html";
    a.textContent = "Back to Season 1";
